...
export default function FeatureAdvertisementPackage() {
  ...
  const fetchFeaturePackages = async () => {
    ...
    try {
      ...
      const response = await fetch("/api/packages?category=feature", {
      ...
      if (response.ok) {
        ...
        if (data.success) {
          // Filter for featured/premium packages
          setPackages(data.data.filter((pkg: any) => 
  ...
  const deleteFeaturePackage = async (packageId: string) => {
    if (!token || !confirm("Are you sure you want to delete this feature package?")) return;
  ...
  const filteredPackages = packages.filter(pkg => {
    ...
    const matchesLevel = selectedLevel === "all" || pkg.featureLevel === selectedLevel;
  ...
  const getFeatureLevelIcon = (level: string) => {
    switch (level) {
      case "spotlight": return <Crown className="h-4 w-4" />;
  ...
  const getFeatureLevelColor = (level: string) => {
    switch (level) {
      case "spotlight": return "bg-yellow-100 text-yellow-800";
  ...
  if (loading) {
    return (
      <div className="text-center py-8">
        ...
        <p className="text-gray-600">Loading feature advertisement packages...</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center">
        <div>
          <h3 className="text-2xl font-bold text-gray-900">Feature Advertisement Packages</h3>
          <p className="text-gray-600">Manage premium featured advertisement packages</p>
        </div>
          ...
          <DialogContent className="max-w-2xl max-h-[80vh] overflow-y-auto">
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-sm font-medium mb-2">Feature Level</label>
                  <Select value={newPackage.featureLevel} onValueChange={(value: any) => setNewPackage({ ...newPackage, featureLevel: value })}>
                    <SelectTrigger>
              ...
              <div className="grid grid-cols-3 gap-4">
                ...
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) => setNewPackage({ ...newPackage, priority: parseInt(e.target.value) || 1 })}
                    placeholder="1"
                    min="1"
                    max="10"
                  />
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="flex items-center space-x-2">
                  <input
                    ...
                    onChange={(e) => setNewPackage({ ...newPackage, bannerPlacement: e.target.checked })}
                  />
                  <label htmlFor="bannerPlacement" className="text-sm">Banner Placement</label>
                </div>
                <div className="flex items-center space-x-2">
                  <input
                    type="checkbox"
                    ...
                    onChange={(e) => setNewPackage({ ...newPackage, socialPromotion: e.target.checked })}
                  />
                  <label htmlFor="socialPromotion" className="text-sm">Social Media Promotion</label>
                </div>
              </div>
              ...
              <div>
                <div className="flex justify-between items-center mb-3">
                  <label className="block text-sm font-medium">Premium Features</label>
                  <Button 
                    type="button" 
                    variant="outline" 
                    size="sm"
              ...
              <div className="flex justify-end space-x-2 pt-4">
                ...
                <Button onClick={createFeaturePackage} className="bg-[#C70000] hover:bg-[#A60000]">
                  Create Feature Package
                </Button>
              </div>
            </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">Feature Packages</CardTitle>
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">From feature packages</p>
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">Avg. Premium Price</CardTitle>
            <Calendar className="h-4 w-4 text-muted-foreground" />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold">
              ₹{packages.length ? Math.round(packages.reduce((sum, pkg) => sum + pkg.price, 0) / packages.length) : 0}
            </div>
              ...
              {filteredPackages.map((pkg) => (
                <TableRow key={pkg._id}>
                  ...
                  <TableCell>
                    ...
                    <p className="text-xs text-gray-500 mt-1">Priority: {pkg.priority}</p>
                  </TableCell>
                  <TableCell>
                    <div>
